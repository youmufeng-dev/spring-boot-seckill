...
public class DesensitizedUtils {
     ...
     * @return
     */
    public static <T> Page desensitizedPage(Page<T> pageContent) {
        //获取page的内容
        List<T> content = pageContent.getRecords();
        if (content == null || content.size() <= 0) {
            return pageContent;
        }
        //数据脱敏
        List list = desensitizedList(content);

        pageContent.setRecords(list);
        return pageContent;
    }
     ...
     * @return
     */
    public static <T> List desensitizedList(List<T> content){
        if (content == null || content.size() <= 0) {
            return content;
        }
